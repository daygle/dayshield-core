...
/// Generate an nftables script that defines and populates a named ban set.
///
/// The script:
/// 1. Creates the `dayshield_crowdsec` table (idempotent via `add table`).
/// 2. Creates a named set of type `ipv4_addr` / `ipv6_addr` (mixed via
///    `interval` flag) inside that table.
/// 3. Flushes any existing elements from the set.
/// 4. Adds every `"ban"`-type decision value as an element.
///
/// Only decisions whose `type_` field equals `"ban"` (case-insensitive) are
/// included.  IPv6 addresses and CIDR ranges are both accepted.
pub fn generate_ban_set_nft(alias_name: &str, decisions: &[CrowdSecDecision]) -> String {
    ...
    // Ensure the set exists (idempotent).
    out.push_str(&format!(
    ...
    // Collect ban-type decision values.
    let bans: Vec<&str> = decisions
        .iter()
...
/// temporary file, and executes `nft -f <tmpfile>`.
///
/// # Errors
///
...
pub async fn update_ban_set(alias_name: &str, decisions: &[CrowdSecDecision]) -> Result<()> {
    ...
    let tmp = "/tmp/dayshield_crowdsec_banset.nft";
    ...
    info!(
        alias_name,
        bans = decisions.iter().filter(|d| d.type_.eq_ignore_ascii_case("ban")).count(),
...
mod tests {
    ...
    fn make_decision(id: u64, value: &str, type_: &str) -> CrowdSecDecision {
        CrowdSecDecision {
            id,

...
fn verify_checksum(file_path: &Path, expected: &str) -> Result<()> {
    // Stream the file through the hasher so memory stays bounded regardless of
    // artifact size (rootfs images can be hundreds of MB).
...
fn verify_artifact_signature(
    ...
    // Verify the signature over the raw 32-byte SHA-256 digest of the artifact
    // (not its hex encoding), matching the documented signing contract
    // `ed25519_sign(privkey, sha256(artifact_bytes))`.  The file is streamed
    // through the hasher to keep memory bounded for large artifacts.
    let mut hasher = Sha256::new();
    {
